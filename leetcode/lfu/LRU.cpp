...
public:
    ...
    void set(int key, int value) {  // set函数
        ...
        else //不存在其中
        {
            ...
            listcaches.push_front(tmpPair);
            cachesMap[key] = listcaches.begin(); //更新map中缓存的值对应的链表迭代器
        }
    }
private:
    int m_capacity;                         //cashe的大小
    list<pair<int, int> > listcaches;           //用一个双链表存储cashe的内容
...
int main(int argc, char **argv)
{
    ...
    {
        key = i ; value = i + 1;
        s.set(key, value);    cout << "set key:" << key << "  value " << value << endl;
