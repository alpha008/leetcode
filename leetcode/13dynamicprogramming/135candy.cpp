...
2. 确定递推公式
    dp[i]只有两个⽅向可以推出来：dp[i - 1] + nums[i]，即： nums[i]加⼊当前连续⼦序列和
    nums[i]，即：从头开始计算当前连续⼦序列和⼀定是取最⼤的，所以dp[i] = max(dp[i - 1] + nums[i], nums[i]);
3. dp数组如何初始化
...
2. 确定递推公式
    dp[i]只有两个⽅向可以推出来：
    dp[i - 1] + nums[i]，即： nums[i]加⼊当前连续⼦序列和nums[i]，即：从头开始计算当前连续⼦序列和
